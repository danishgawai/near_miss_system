...
class TrackState:
    cls_name: str
    age: int = 0
    positions: deque = field(default_factory=lambda: deque(maxlen=40))   # BEV meters
    img_points: deque = field(default_factory=lambda: deque(maxlen=40))  # image points
...
class MotionEstimator:
    def __init__(self, fps: float, velocity_alpha: float, accel_alpha: float):
        ...
        self.v_alpha = velocity_alpha
        self.a_alpha = accel_alpha
        self.prev_gray = None
    ...
    def refine_velocity_with_flow(
        ...
        p1, st, _ = cv2.calcOpticalFlowPyrLK(self.prev_gray, gray, p0, None, winSize=(15, 15), maxLevel=2)
        self.prev_gray = gray
        ...
        flow = p1[0][0] - p0[0][0]
        fn = np.linalg.norm(flow)
        bn = np.linalg.norm(base_vel)
        if fn < 0.5 or bn < 1e-6:
            return base_vel

        flow_dir = flow / (fn + 1e-6)
        base_dir = base_vel / (bn + 1e-6)
        align = float(np.clip(np.dot(flow_dir, base_dir), -1.0, 1.0))
        gain = max(0.0, align) * 0.15
        return base_vel * (1.0 + gain)
